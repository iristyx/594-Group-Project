...
public class UserInterface {
	...
	public UserInterface(ParkingProcessor parkingProcessor, PopulationProcessor populationProcessor,
		...
		while (true) {
			...
			if (choice.equals("0")) {
				...
				throw new IllegalArgumentException(
						"Invalid input! Input must be an integer 1-6, or 0 to exit program.");
			}
	...
	/**
	 * Task 1: Display the total population of all of the ZIP Codes in the
	 * population input file Should return 1526206 based on provided input file
	 * 'population.txt'
	 */
	...
	/**
	 * Task 2: Display the total fines per capita for each ZIP Code i.e. total
	 * aggregates fines divided by population of the ZIP Code ZIP Codes must be
	 * written to screen in ascending numerical order Total fines per capita must be
	 * displayed with precision of four decimal places (truncated, not rounded)
	 */
	protected void doTotalParkingFinesPerCapita() {

		// If computation was not performed before
		if (!results.containsKey("2")) {
			Map<String, Double> totalParkingFinesPerCapita = parkingProcessor
					.getTotalFinesPerCapitaForZipCodesInState("pa");
			Map<String, Double> sortedTotalParkingFinesPerCapita = new TreeMap<String, Double>(
	...
	/**
	 * Task 3: Display the average residential market value for user-specified ZIP
	 * Code i.e. total market value for all residences in the ZIP Code divided by
	 * the number of residences Result must be truncated to an integer, not rounded
	 * Display '0' if total residential market value for ZIP Code is 0 or if user
	 * inputs invalid ZIP Code
	 */
	protected void doAverageMarketValueForResidences() {
		...
		if (!propertyProcessor.getPropertiesZipCodes().contains(zipCode)) {
			resultsMap.put(zipCode, 0.0);
		}
		...
		// If ZIP Code is valid
			...
			// If computation was performed before, check if it was done for the current
			// input ZIP Code
			else {
	...
	/**
	 * Task 4: Display the average residential livable area for user-specified ZIP
	 * Code i.e. sum of total livable areas for all residences in the ZIP Code
	 * divided by the number of residences Result must be truncated to an integer,
	 * not rounded Display '0' if total residential livable area for ZIP Code is 0
	 * or if user inputs invalid ZIP Code
	 */
	...
	/**
	 * Task 5: Display the total residential market value per capita for
	 * user-specified ZIP Code Result must be truncated to an integer, not rounded
	 * Display '0' if total residential market value for ZIP Code is 0 or if user
	 * inputs invalid ZIP Code
	 */
	protected void doTotalResidentialMarketValuePerCapita() {
		...
		// If ZIP Code is invalid
		if (!propertyProcessor.getValidZipCodes().contains(zipCode)) {
			resultsMap.put(zipCode, 0.0);
		}
	...
	/**
	 * Task 6: Display the residential market value per capita over number of fines
	 * for a user-specified ZIP Code Result truncated to an integer, not rounded
	 * Display '0' if total residential market value for ZIP Code is 0, if number of
	 * fines is 0, or if user inputs invalid ZIP Code
	 */
	protected void doPerCapitaMarketValuePerFineByZipCode() {
		...
		// If ZIP Code is invalid
		if (!propertyProcessor.getValidZipCodes().contains(zipCode)
				|| !parkingProcessor.getStateZipCodes("pa").contains(zipCode)) {
			resultsMap.put(zipCode, 0.0);
		}
	...
	/*
	 * Prompt user to input 5-digit ZIP Code in console Return 5-digit string of
	 * valid ZIP Code, or '0' if invalid input
	 */
	...
	/*
	 * Truncate input double to specified number of decimalPlaces
	 */
